...
try the stupid mediationa analysis

```{r winning model mediation test,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = FALSE}
data(jobs)

f1 <- bf(job_seek ~ treat + econ_hard + sex + age)
f2 <- bf(depress2 ~ treat + job_seek + econ_hard + sex + age)
med2 <- brm(f1 + f2 + set_rescor(FALSE),
          data = jobs, 
          file = 'med_testmodel_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4)
