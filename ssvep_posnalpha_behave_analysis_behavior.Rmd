...
summary(post_hocs.side$contrasts) %>%
  ...
  # mutate(CohensD = `t.ratio`/sqrt(df+1))%>%
  mutate(`CohensD` =
           summary(eff_size(contrast(post_hocs.side, method = "pairwise", adjust = "holm"),
                            sigma = sqrt(mean(sigma(StatsOut$lm)^2)), # RMS of sigma()
                            edf = df.residual(StatsOut$lm), method = "identity"))$effect.size
  )%>%
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), 
...
summary(post_hocs.valXside$contrasts) %>%
  ...
  # mutate(CohensD = `t.ratio`/sqrt(df+1))%>%
  mutate(`CohensD` =
           summary(eff_size(contrast(post_hocs.valXside, method = "pairwise", adjust = "holm"),
                            sigma = sqrt(mean(sigma(StatsOut$lm)^2)), # RMS of sigma()
                            edf = df.residual(StatsOut$lm), method = "identity"))$effect.size
  )%>%
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), 
...
StatsData <- DATA.In_long %>%
  ...
  ungroup()%>%
  group_by(subject, cue_validity_label)%>%
  summarise(n_resp = sum(n_resp), mean_RT = mean(mean_RT), n_cons = sum(n_cons), rate_perc = mean(rate_perc))%>%
  dplyr::select(-n_resp, -n_cons, -rate_perc)%>%
  pivot_wider(names_from = cue_validity_label, values_from = mean_RT)%>%
  mutate(valid_MINUS_neutral = valid - neutral, valid_MINUS_invalid = valid - invalid, neutral_MINUS_invalid = neutral - invalid)%>%
  dplyr::select(-invalid, -neutral, -valid)%>%
  pivot_longer(cols = valid_MINUS_neutral:neutral_MINUS_invalid, names_to = "contrast", values_to = "RT_diff")
...
StatsData %>%
  ...
  mutate(meanval = map(data, ~ mean(.x$RT_diff))) %>%
  mutate(std = map(data, ~ sd(.x$RT_diff))) %>%
  dplyr::select(-data) %>%
  unnest(cols = c(stats, meanval, std))%>%
  ungroup()%>%
  dplyr::select(contrast, meanval, std, statistic, parameter, p.value)%>%
  ...
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), caption = c("classical approach: t-tests against 0 | dv = RT difference ")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
summary(post_hocs.side$contrasts) %>%
  ...
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), 
        caption = c("contrasts for estimated marginal means | by side | dv = HR | corrected: Holm")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
summary(post_hocs.valXside$contrasts) %>%
  ...
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), 
        caption = c("contrasts for estimated marginal means | by validity and side | dv = HR | corrected: Holm")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
StatsData <- DATA.In_long %>%
  ...
  group_by(subject, cue_validity_label)%>%
  summarise(n_resp = sum(n_resp), mean_RT = mean(mean_RT), n_cons = sum(n_cons), rate_perc = mean(rate_perc))%>%
  dplyr::select(-n_resp, -n_cons, -mean_RT)%>%
  pivot_wider(names_from = cue_validity_label, values_from = rate_perc)%>%
  mutate(valid_MINUS_neutral = valid - neutral, valid_MINUS_invalid = valid - invalid, neutral_MINUS_invalid = neutral - invalid)%>%
  dplyr::select(-invalid, -neutral, -valid)%>%
  pivot_longer(cols = valid_MINUS_neutral:neutral_MINUS_invalid, names_to = "contrast", values_to = "HR_diff")
...
StatsData %>%
  ...
  mutate(meanval = map(data, ~ mean(.x$HR_diff))) %>%
  mutate(std = map(data, ~ sd(.x$HR_diff))) %>%
  dplyr::select(-data) %>%
  unnest(cols = c(stats, meanval, std))%>%
  ungroup()%>%
  dplyr::select(contrast, meanval, std, statistic, parameter, p.value)%>%
  ...
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), caption = c("t-tests against 0 | dv = HR difference ")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
