...
plotttitle <- ggdraw() + draw_label("motor alpha modulation | induced", fontface='bold')
...
### Post-hoc t-Tests electrophysiological measures (cued vs neutral vs uncued)  
based on classical paired t-test approach
...
DATA.In_longer %>%
  ...
  dplyr::select(signal, attention, meanval, std,  statistic, parameter, p.value)%>%
  group_by(signal)%>%
  ...
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), caption = c("t-tests for tests agains zero | Holm-corrected")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))



# non-emmeans version contrasts
DATA.In_longer %>%
  filter(post_hit == "hit") %>%
  dplyr::select(subject, trialnumber, post_event_pos_label, signal, time, amplitude, attention)%>%
  group_by(subject, post_event_pos_label, signal, time, attention) %>%
  ...
  ungroup()%>%
  filter(time == "pre_to_post")%>%
  # filter(time == "[500 1500]ms")%>%
  pivot_wider(names_from = "attention", values_from = "amplitude")%>%
  mutate("uncued-neutral" = uncued-neutral, "uncued-cued" = uncued - cued, "neutral-cued" = neutral - cued)%>%
  dplyr::select(-uncued, -neutral, -cued)%>%
  pivot_longer(cols = c("uncued-neutral","uncued-cued", "neutral-cued"), names_to = "pair", values_to = "amplitude")%>%
  ...
  dplyr::select(-data) %>%
  unnest(cols = c(stats, meanval, std))%>%
  dplyr::select(signal, pair, meanval, std,  statistic, parameter, p.value)%>%
  group_by(signal)%>%
  ...
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), caption = c("t-tests for contrasts | Holm-corrected")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
### SSVEP and behavior correlation


```{r correlation - estimation, echo=FALSE, message=FALSE, fig.height=3, fig.width=3}
def.cormethod ="spearman"
num.iter = 10000


corr_behavior <- DATA.In_longer %>%
  dplyr::select(subject, trialnumber, cue_validity_label, post_event_pos_label, post_hit, post_RT)%>%
  filter(post_hit=="hit")%>%
  group_by(subject, trialnumber,cue_validity_label)%>%
  summarise(post_RT=mean(post_RT))%>%
...
corr_SSVEP <- DATA.In_longer %>%
  ...
  group_by(subject, cue_validity_label, pos_rel_target)%>%
  dplyr::summarise(Mean_SSVEP = mean(amplitude))%>%
  ungroup()%>%
  mutate(cue_validity_label = factor(cue_validity_label, levels=c('invalid','neutral','valid'), ordered = T))

corrdata <- merge(corr_SSVEP, corr_behavior, by = c('subject','cue_validity_label')) %>%
  filter(pos_rel_target=="contra_target", cue_validity_label=="valid")

# calculate correlations
# calculate confidence intervals
corrRho = SpearmanRho(x = corrdata$Mean_SSVEP, y = corrdata$Mean_RT, conf.level = .95)
cbf = correlationBF(corrdata$Mean_SSVEP, corrdata$Mean_RT, rscale = "medium",iterations=num.iter)

### with brand new
# van Doorn, J., Ly, A., Marsman, M., and Wagenmakers, E.-J. (2020). Bayesian rank-based hypothesis testing for the rank sum test, the signed rank test, and Spearman’s ρ. J. Appl. Stat. 47, 2984–3006. doi:10.1080/02664763.2019.1709053.
rhoSamples <- spearmanGibbsSampler(xVals = corrdata$Mean_SSVEP,
                                   yVals = corrdata$Mean_RT, 
                                   nSamples = 5e3)
# Give the posterior samples for rho to the function below to compute BF10
cbfSP = computeBayesFactorOneZero(rhoSamples$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)


ggscatter(corrdata, x = "Mean_SSVEP", y = "Mean_RT", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = def.cormethod, size = 3, alpha = 0.6, cor.coef.size = 3, cor.coeff.args = list(digits = 3),
          add.params = list(fill = "lightgray"),
          ggtheme = theme_bw(),
          xlab = "target SSVEP pre-to-post mod | valid", ylab = "RT in ms | valid",
          # title = sprintf("Rho: %1.3f CI[%1.3f %1.3f]; BF: %1.3f",corrRho1[[1]],corrRho1[[2]],corrRho1[[3]],extractBF(cbf1)[[1]])
          title = sprintf("Rho: %1.3f CI[%1.3f %1.3f]; BF: %1.3f",corrRho[[1]],corrRho[[2]],corrRho[[3]],cbfSP)
)+
  font("title", size = 8)
...
corrdata <- merge(corr_SSVEP, corr_behavior, by = c('subject','cue_validity_label')) %>%
  filter(pos_rel_target=="contra_target") %>%
  dplyr::select(-Median_RT, -SD_RT)%>%
  pivot_wider(names_from = "cue_validity_label", values_from = c("Mean_SSVEP", "Mean_RT")) %>%
  mutate(SSVEP_valid_Min_invalid = Mean_SSVEP_valid-Mean_SSVEP_invalid)%>%
  mutate(RT_valid_Min_invalid = Mean_RT_valid-Mean_RT_invalid)

# calculate correlations
# calculate confidence intervals
corrRho = SpearmanRho(x = corrdata$SSVEP_valid_Min_invalid, y = corrdata$RT_valid_Min_invalid, conf.level = .95)
cbf = correlationBF(corrdata$SSVEP_valid_Min_invalid, corrdata$RT_valid_Min_invalid, rscale = "medium",iterations=num.iter)

### with brand new
# van Doorn, J., Ly, A., Marsman, M., and Wagenmakers, E.-J. (2020). Bayesian rank-based hypothesis testing for the rank sum test, the signed rank test, and Spearman’s ρ. J. Appl. Stat. 47, 2984–3006. doi:10.1080/02664763.2019.1709053.
rhoSamples <- spearmanGibbsSampler(xVals = corrdata$SSVEP_valid_Min_invalid,
                                   yVals = corrdata$RT_valid_Min_invalid, 
                                   nSamples = 5e3)
# Give the posterior samples for rho to the function below to compute BF10
cbfSP = computeBayesFactorOneZero(rhoSamples$rhoSamples, 
                          whichTest = "Spearman",
...
ggscatter(corrdata, x = "SSVEP_valid_Min_invalid", y = "RT_valid_Min_invalid", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = def.cormethod, size = 3, alpha = 0.6, cor.coef.size = 3, cor.coeff.args = list(digits = 3),
          add.params = list(fill = "lightgray"),
          ggtheme = theme_bw(),
          xlab = "diff SSVEP pre-to-post mod | valid - invalid", ylab = "diff RT in ms | valid - invalid",
          # title = sprintf("Rho: %1.3f CI[%1.3f %1.3f]; BF: %1.3f",corrRho1[[1]],corrRho1[[2]],corrRho1[[3]],extractBF(cbf1)[[1]])
          title = sprintf("Rho: %1.3f CI[%1.3f %1.3f]; BF: %1.3f",corrRho[[1]],corrRho[[2]],corrRho[[3]],cbfSP)
)+
  font("title", size = 8)
...
### **estimating models | ndt/shift vary across subjects**
<br>

as above but having ndt/shift vary across subjects
...
# intercept model
fit_ndt_intercept = brm(formula = bf(post_RT ~ 1 + (1|subject), ndt ~ 1 + (1|subject)),
          data = modeldata, 
          family = shifted_lognormal(),
          file = 'fit_ndt_intercept_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# validity model
fit_ndt_validity = brm(formula = bf(post_RT ~ cue_validity_label + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target
fit_ndt_visalpha = brm(formula = bf(post_RT ~ post_cue_vis_alpha_contra_target + (1|subject), ndt ~ 1 + (1|subject)),
          data = modeldata2, 
          family = shifted_lognormal(),
          file = 'fit_ndt_visalpha_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target + validity
fit_ndt_validity_visalpha = brm(
  formula = bf(post_RT ~ cue_validity_label + post_cue_vis_alpha_contra_target + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata2, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_visalpha_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target * validity
fit_ndt_validity_x_visalpha = brm(
  formula = bf(post_RT ~ cue_validity_label * post_cue_vis_alpha_contra_target + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata2, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_x_visalpha_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra nontarget 
fit_ndt_alpha2 = brm(formula = bf(post_RT ~ post_cue_vis_alpha_contra_nontarget + (1|subject), ndt ~ 1 + (1|subject)),
          data = modeldata3, 
          family = shifted_lognormal(),
          file = 'fit_ndt_visalpha2_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          # file_refit = "always",
          cores = 4
...
# visual alpha contra nontarget + validity
fit_ndt_validity_alpha2 = brm(
  formula = bf(post_RT ~ cue_validity_label + post_cue_vis_alpha_contra_nontarget + (cue_validity_label|subject), 
               ndt ~ 1 + (1|subject)),
          data = modeldata3, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_visalpha2_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra nontarget + validity
fit_ndt_validity_x_alpha2 = brm(
  formula = bf(post_RT ~ cue_validity_label * post_cue_vis_alpha_contra_nontarget + (cue_validity_label|subject), 
               ndt ~ 1 + (1|subject)),
          data = modeldata3, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_x_visalpha2_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target and contra non-target
fit_ndt_alpha1_alpha2 = brm(
  formula = bf(post_RT ~ contra_target + contra_nontarget + (1|subject), ndt ~ 1 + (1|subject)),
          data = modeldata4, 
          family = shifted_lognormal(),
          file = 'fit_ndt_visalpha1_visalpha2_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target and contra non-target + validity
fit_ndt_validity_alpha1_alpha2 = brm(
  formula = bf(post_RT ~ cue_validity_label + contra_target + contra_nontarget + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata4, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_visalpha1_visalpha2_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target and contra non-target + validity
fit_ndt_validity_x_alpha1_alpha2 = brm(
  formula = bf(post_RT ~ cue_validity_label * contra_target + cue_validity_label * contra_nontarget + (cue_validity_label|subject),
                ndt ~ 1 + (1|subject)),
          data = modeldata4, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_x_visalpha1_visalpha2_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# SSVEP model
fit_ndt_SSVEP = brm(
  formula = bf(post_RT ~ post_cue_SSVEP_contra_target + (1|subject), ndt ~ 1 + (1|subject)),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_ndt_ssvep_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# SSVEP + validity model
fit_ndt_validity_SSVEP = brm(
  formula = bf(post_RT ~ cue_validity_label + post_cue_SSVEP_contra_target + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_ssvep_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
fit_ndt_validity_x_SSVEP = brm(
  formula = bf(post_RT ~ cue_validity_label * post_cue_SSVEP_contra_target + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_x_ssvep_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target and contra non-target and SSVEP
fit_ndt_SSVEP_alpha = brm(
  formula = bf(post_RT ~ visualAlpha_contra_target + visualAlpha_contra_nontarget + SSVEP_contra_target + (1|subject),
                ndt ~ 1 + (1|subject)),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_ndt_SSVEP_alpha_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
# visual alpha contra target and contra non-target and SSVEP  + validity
fit_ndt_validity_SSVEP_alpha = brm(
  formula = bf(post_RT ~ cue_validity_label + visualAlpha_contra_target + visualAlpha_contra_nontarget + SSVEP_contra_target + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_SSVEP_alpha_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
fit_ndt_validity_x_SSVEP_alpha = brm(
  formula = bf(post_RT ~ cue_validity_label * visualAlpha_contra_target + cue_validity_label * visualAlpha_contra_nontarget + cue_validity_label * SSVEP_contra_target + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_ndt_validity_x_SSVEP_alpha_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          cores = 4
          # file_refit = "always"
...
``` {r brms model for RTs 03b eval, fig.height=6, fig.width=10, warning=FALSE, eval = TRUE}
#define priors: [needs rechecking!]
# prior_summary(fit_visalpha) to show priors
...
get_prior(formula = bf(post_RT ~ cue_validity_label + (cue_validity_label|subject), ndt ~ 1 + (1|subject)),
  data = modeldata2, 
  family = shifted_lognormal()
)

get_prior(formula = bf(post_RT ~ cue_validity_label + (cue_validity_label|subject)),
  data = modeldata2, 
  family = shifted_lognormal()
)
...
testfit_intecept_noPRIORs <- add_criterion(testfit_intecept_noPRIORs, c("loo", "waic", "bayes_R2"))
testfit_intecept_withPRIORs <- add_criterion(testfit_intecept_withPRIORs, c("loo", "waic", "bayes_R2"))
testfit_intecept_ndt_noPRIORs <- add_criterion(testfit_intecept_ndt_noPRIORs, c("loo", "waic", "bayes_R2"))
testfit_intecept_ndt_withPRIORs <- add_criterion(testfit_intecept_ndt_withPRIORs, c("loo", "waic", "bayes_R2"))
testfit_validity_noPRIORs <- add_criterion(testfit_validity_noPRIORs, c("loo", "waic", "bayes_R2"))
testfit_validity_withPRIORs <- add_criterion(testfit_validity_withPRIORs, c("loo", "waic", "bayes_R2"))
testfit_validity_ndt_noPRIORs <- add_criterion(testfit_validity_ndt_noPRIORs, c("loo", "waic", "bayes_R2"))
testfit_validity_ndt_withPRIORs <- add_criterion(testfit_validity_ndt_withPRIORs, c("loo", "waic", "bayes_R2"))


plot(testfit_intecept_noPRIORs, ask = FALSE)
# plot(testfit_intecept_withPRIORs, ask = FALSE)
plot(testfit_intecept_ndt_noPRIORs, ask = FALSE)
# plot(testfit_intecept_ndt_withPRIORs, ask = FALSE)
plot(testfit_validity_noPRIORs, ask = FALSE)
plot(conditional_effects(testfit_validity_noPRIORs), ask = FALSE)
plot(testfit_validity_withPRIORs, ask = FALSE)
plot(conditional_effects(testfit_validity_withPRIORs), ask = FALSE)
plot(testfit_validity_ndt_noPRIORs, ask = FALSE)
plot(conditional_effects(testfit_validity_ndt_noPRIORs), ask = FALSE)
# plot(testfit_validity_ndt_withPRIORs, ask = FALSE)
# plot(conditional_effects(testfit_validity_ndt_withPRIORs), ask = FALSE)


# all models with validity as predictor
loo_compare(testfit_intecept_noPRIORs, testfit_intecept_withPRIORs, testfit_intecept_ndt_noPRIORs,
            testfit_validity_noPRIORs, testfit_validity_withPRIORs, testfit_validity_ndt_noPRIORs,
            criterion = c("loo"))%>%
  kable(escape = F, digits = c(6,6,6,6,6,6,6,6,6), caption = c("intercept and validity models | prior or not | loo comparison")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))

as.data.frame(rbind(
  bayes_R2(testfit_intecept_noPRIORs), bayes_R2(testfit_intecept_withPRIORs), bayes_R2(testfit_intecept_ndt_noPRIORs),
  bayes_R2(testfit_validity_noPRIORs), bayes_R2(testfit_validity_withPRIORs), bayes_R2(testfit_validity_ndt_noPRIORs)),
  row.names = c("testfit_intecept_noPRIORs","testfit_intecept_withPRIORs","testfit_intecept_ndt_noPRIORs",
                "testfit_validity_noPRIORs", "testfit_validity_withPRIORs", "testfit_validity_ndt_noPRIORs"))%>%
  kable(escape = F, digits = c(6,6,6,6,6,6,6,6,6), caption = c("intercept and validity models | prior or not | bayes_R2")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
