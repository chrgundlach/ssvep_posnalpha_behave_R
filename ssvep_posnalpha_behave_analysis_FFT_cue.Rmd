...
visual alpha models
contra target                         **RT ~ visual alpha + ( 1 | subjects)**
contra target + validity              **RT ~ Cue + visual alpha + ( cue | subjects)**
contra nontarget                      **RT ~ visual alpha + ( 1 | subjects)**
contra nontarget + validity           **RT ~ Cue + visual alpha + ( cue | subjects)**
...
with z-scored alpha and SSVEP amplitudes
...
# visual alpha contra target and contra non-target and SSVEP  + validity
fit_z_validity_SSVEP_alpha = brm(formula = post_RT ~ cue_validity_label + visualAlpha_contra_target + visualAlpha_contra_nontarget + SSVEP_z_contra_target + (cue_validity_label|subject),
          data = modeldata5, 
          family = shifted_lognormal(),
...
fit_z_validity_x_SSVEP_alpha = brm(
  formula = post_RT ~ cue_validity_label * visualAlpha_contra_target + cue_validity_label * visualAlpha_contra_nontarget + cue_z_validity_label * SSVEP_contra_target + (cue_validity_label|subject),
          data = modeldata5, 
          family = shifted_lognormal(),
...
# best SSVEP and best alpha models and combined
loo_compare(fit_intercept, fit_validity, fit_validity_alpha1_alpha2, fit_validity_x_alpha1_alpha2, fit_validity_SSVEP,
            fit_validity_x_SSVEP, fit_validity_SSVEP_alpha, fit_validity_x_SSVEP_alpha, criterion = c("loo"))%>%
  kable(escape = F, digits = c(6,6,6,6,6,6,6,6,6), caption = c("alpha, ssvep and combined models | loo comparison")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))

as.data.frame(rbind(
  bayes_R2(fit_intercept), bayes_R2(fit_validity), bayes_R2(fit_validity_alpha1_alpha2), bayes_R2(fit_validity_x_alpha1_alpha2),
  bayes_R2(fit_validity_SSVEP), bayes_R2(fit_validity_x_SSVEP), bayes_R2(fit_validity_SSVEP_alpha), bayes_R2(fit_validity_x_SSVEP_alpha)),
  row.names = c("fit_intercept","fit_validity","fit_validity_alpha1_alpha2", "fit_validity_x_alpha1_alpha2","fit_validity_SSVEP",
                "fit_validity_x_SSVEP", "fit_validity_SSVEP_alpha", "fit_validity_x_SSVEP_alpha"))%>%
...
#### use it to predict data

```{r predict datas, fig.height=6, fig.width=10, warning=FALSE, eval = TRUE}
# it seems as the model with visual alpha contra to the target and contra to the non target is superior?
# alter data to extract post-cue alpha contra to target
plot(fit_validity_alpha1_alpha2, ask = FALSE)
plot(conditional_effects(fit_validity_alpha1_alpha2), ask = FALSE)

# extract all fixed effects (and confidence intervals) that are relevant for the model
# we basically modelled the fixed effects to affect the difficulty, mu-parameter
tidy(fit_validity_alpha1_alpha2, effects = c("fixed"),
     conf.int = TRUE, conf.level = 0.95)
fixef(fit_validity_alpha1_alpha2)

# other parameters relevant for the response time curves modelled as a shifted lognormal distribution: ndt = shift; sigma = scale
# can be extracted from the summary function (with confidence intervals)
summary(fit_validity_alpha1_alpha2)
posterior_summary(as.data.frame(fit_validity_alpha1_alpha2)$sigma)
posterior_summary(as.data.frame(fit_validity_alpha1_alpha2)$ndt)

# predict RTs with model
bestmodel.modeldata <- fit_validity_alpha1_alpha2$data
# with all posteriors
# bestmodel.predict <- posterior_predict(fit_validity_alpha1_alpha2)
# summarized
bestmodel.predict <- predict(fit_validity_alpha1_alpha2)
# combine

bestmodel.modeldata <- cbind(bestmodel.modeldata, as.data.frame(bestmodel.predict))
...
```{r plt attempts 1, fig.height=6, fig.width=10, warning=FALSE, eval = TRUE}
