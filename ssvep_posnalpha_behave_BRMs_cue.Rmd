...
dat_plot <- DATA.In_longer %>%
  dplyr::select(subject, trialnumber, cue_validity_label, post_event_pos_label, post_hit, amplitude, signal, pos_rel_target, time)%>%
  filter(post_hit=="hit", signal == "motorAlpha", pos_rel_target == "contra_target", time == "[500 1500]ms")
...
# cowplot::plot_grid(plot1, plot2, plot3, ncol = 3,labels = "AUTO")
plotttitle <- ggdraw() + draw_label("motor alpha-band amplitude contra target", fontface='bold')
plotplots <- ggarrange(plot1, plot2, nrow = 2, align = "v", heights =c(4,2), common.legend = TRUE)
ggarrange(plotttitle, plotplots, nrow = 2, heights=c(0.1, 1))
...
* contra target: **RT ~ motor_alpha_Targ + ( 1 | subjects)**
* contra target + validity: **RT ~ Cue +  motor_alpha_Targ + ( cue | subjects)**
...
modeldata6 = DATA.In_longer %>%
  ...
  mutate(target_malpha_contra_target = amplitude)

# motor alpha contra target
fit_motoralpha = brm(formula = post_RT ~ target_malpha_contra_target + (1|subject),
          data = modeldata6, 
          family = shifted_lognormal(),
...
fit_validity_motoralpha = brm(formula = post_RT ~ cue_validity_label + target_malpha_contra_target + (cue_validity_label|subject),
          data = modeldata6, 
          family = shifted_lognormal(),
...
# motor alpha contra target * validity
fit_validity_x_motoralpha = brm(formula = post_RT ~ cue_validity_label * target_malpha_contra_target + (cue_validity_label|subject),
          data = modeldata6, 
          family = shifted_lognormal(),
