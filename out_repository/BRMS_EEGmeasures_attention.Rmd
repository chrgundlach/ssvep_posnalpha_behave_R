...
# just do the intercept model

fit_EEG_vis_alpha_intercept = brm(formula = visalpha ~ 1 + (1|subject),
          data = modeldata2, 
          family = shifted_lognormal(),
          file = 'fit_EEGv2_vis_alpha_targ_intercept_slog',
...
modeldata2b = DATA.In_longer %>%
  filter(post_hit == "hit", signal == "visualAlpha", time == "[500 1500]ms")%>%
  mutate(visalpha = amplitude) %>%
  dplyr::select(pos_rel_target, cue_validity_label, visalpha, subject, trialnumber) %>%
  pivot_wider(names_from = pos_rel_target, values_from = visalpha)

fit_EEG_vis_alpha_contripsi_validity = brm(formula = contra_target ~ 1 + cue_validity_label*contra_nontarget + (cue_validity_label|subject),
          data = modeldata2b, 
          family = shifted_lognormal(),
          file = 'fit_EEGv2_vis_alpha_contripsi_targ_validity_slog',
          save_pars = save_pars(all = TRUE),
          # file_refit = "on_change",
          # file_refit = "always",
          file_refit = "never",
...
fit_EEG_SSVEP_intercept = brm(formula = SSVEP ~ 1 + (1|subject),
          data = modeldata3, 
          family = shifted_lognormal(),
...
fit_EEG_vis_alpha_contripsi_validity <- add_criterion(fit_EEG_vis_alpha_contripsi_validity, c("loo", "waic", "bayes_R2"))
...
as.data.frame(rbind(
  bayes_R2(fit_EEG_vis_alpha_intercept),
  bayes_R2(fit_EEG_mot_alpha_intercept),
  bayes_R2(fit_EEG_SSVEP_intercept),
  bayes_R2(fit_EEG_vis_alpha_validity),
  bayes_R2(fit_EEG_mot_alpha_validity),
  bayes_R2(fit_EEG_SSVEP_validity_x_alpha)),
  row.names = c( "fit_EEG_vis_alpha_intercept", 
                "fit_EEG_mot_alpha_intercept", 
                "fit_EEG_SSVEP_intercept",
                "fit_EEG_vis_alpha_validity", 
...
summary(fit_EEG_vis_alpha_intercept)
summary(fit_EEG_mot_alpha_intercept)
summary(fit_EEG_SSVEP_intercept)
summary(fit_EEG_vis_alpha_validity)
summary(fit_EEG_vis_alpha_contripsi_validity)
...
##  Extract marginal effects from alpha complex model alpha contra ipsi model


```{r alpha compl model marginal effects disp effects,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# based on https://www.andrewheiss.com/blog/2021/11/10/ame-bayes-re-guide/
# which model to use?
bestmodel.ef.model <- fit_EEG_vis_alpha_contripsi_validity
...
# use range of data from orignal study 
bestmodel.ef.newdata <- expand_grid(
  cue_validity_label  = as.factor(c('invalid','neutral','valid')),
  contra_nontarget = median(bestmodel.ef.data$contra_nontarget),
  # post_cue_vis_alpha_contra_target = median(bestmodel.ef.data$post_cue_vis_alpha_contra_target),
  # contra_nontarget = median(bestmodel.ef.data$contra_nontarget),
  # contra_target = seq(min(bestmodel.ef.data$contra_target),max(bestmodel.ef.data$contra_target),1),
...
bestmodel.ef.model %>% 
  emmeans(~ cue_validity_label,
          epred = TRUE)%>%
  kable(escape = F, digits = c(3,3,3,6,3,3,3,3,3), 
        caption = c("average marginal effect | point measure = median of alpha contra target")) %>%
  # kable_classic(full_width = F) %>%
  column_spec(1, bold = T)%>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
bestmodel.ef.model %>% 
  ...
  contrast(method = "revpairwise")%>%
  kable(escape = F, digits = c(3,3,3,6,3,3,3,3,3), 
        caption = c("average marginal effect differences | point measure = median of alpha contra target")) %>%
  # kable_classic(full_width = F) %>%
  column_spec(1, bold = T)%>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
g <-ggplot(bestmodel.ef.ef_draws_validity, aes(y = cue_validity_label, x = .value)) +
  ...
  scale_color_manual(values=c("#F1831A", "#293C4A", "#198A83"))+
  labs(
    title = "Average predicted alpha contra target amplitude", 
    subtitle = "based on medians for all draws of the model",
    fill_ramp = "interval",
    x = expression(paste("amplitude in ", mu, "V")),
    y = "validity"
...
```{r SSVEP compl marginal effects disp effects visual alpha -marginal means,  fig.height=6, fig.width=4,  warning = FALSE, message=FALSE, eval = TRUE}
# now trying to get the regression line right for the alpha predictors
...
theme_set(theme_bw())
ggplot(bestmodel.ef.tidy_exppred2, aes(x = contra_nontarget, y = .epred)) +
  stat_lineribbon(.width = c(0.5, 0.8, 0.95, 1)) + 
  scale_fill_brewer(palette = "Reds") +
  facet_grid(cue_validity_label~.)+
  labs(x = "alpha amplitude contra nontarget", y = "predicted alpha contra target",
       fill = "Credible interval") +
  theme(legend.position = "bottom")
