...
datafile1 <- "C:/Users/EEG/Documents/R/Christopher/ssvep_posnalpha_behave_R/data_in/Behavior_FFT_singletrials_cue.txt"
...
# visual alpha contra target and contra non-target x validity
...
# visual alpha contra target and contra non-target x validity + SSVEP x validity
fit_validity_x_alpha1_alpha2_SSVEP = brm(
  formula = post_RT ~ cue_validity_label * visualAlpha_contra_target 
  + cue_validity_label * visualAlpha_contra_nontarget
  + cue_validity_label * SSVEP_contra_target + (cue_validity_label|subject),
          data = modeldata8, 
          family = shifted_lognormal(),
          file = 'fit_validity_x_visalpha1_visalpha2_SSVEP_slog',
          save_pars = save_pars(all = TRUE),
          # file_refit = "on_change",
          # file_refit = "always",
          file_refit = "never",
...
fit_validity_x_alpha1_alpha2_SSVEP <- add_criterion(fit_validity_x_alpha1_alpha2_SSVEP, c("loo", "waic", "bayes_R2"))
...
# summary(fit_validity_x_alpha1_alpha2_SSVEP)
...
###  Extract marginal effects for post-cue visual alpha and SSVEP interaction model target + nontarget + SSVEP target

Now for illustrating the whole picture, lets plot the motor alpha model

**'RT ~ Cue * vis_alpha_contratarg + Cue * vis_alpha_contranontarg + Cue * SSVEP + ( cue | subjects)'**

```{r visalphainter contra ipsi SSVEP model pre marginal effects disp effects,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# based on https://www.andrewheiss.com/blog/2021/11/10/ame-bayes-re-guide/
# which model to use?
bestmodel.ef.model <- fit_validity_x_alpha1_alpha2_SSVEP
# what is the data of the model?
bestmodel.ef.data <- bestmodel.ef.model$data
...
```{r visalphainter SSVEP model marginal effects disp effects VALIDITY -data ,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# use range of data from orignal study 
bestmodel.ef.newdata3 <- expand_grid(
  cue_validity_label = c('valid','neutral','invalid'),
  visualAlpha_contra_target = median(bestmodel.ef.data$visualAlpha_contra_target),
  visualAlpha_contra_nontarget = median(bestmodel.ef.data$visualAlpha_contra_nontarget),
  SSVEP_contra_target = median(bestmodel.ef.data$SSVEP_contra_target),
  # contra_nontarget = median(bestmodel.ef.data$contra_nontarget),
  # post_cue_vis_alpha_contra_target = seq(min(bestmodel.ef.data$post_cue_vis_alpha_contra_target),max(bestmodel.ef.data$post_cue_vis_alpha_contra_target),1),
  # contra_nontarget = seq(min(bestmodel.ef.data$contra_nontarget),max(bestmodel.ef.data$contra_nontarget),1),
  # subject = as.factor(unique(bestmodel.ef.data$subject))
...
```{r visalphainterSSVEP model marginal effects disp effects VALIDITY -marginal means,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
...
```{r visalphainterSSVEP model marginal effects disp effects visual alpha inter -marginal means,  fig.height=4, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# now trying to get the regression line right for the alpha predictors
...
bestmodel.ef.newdata2 <- expand_grid(
  ...
  # contra_target = median(bestmodel.ef.data$contra_target),
  visualAlpha_contra_nontarget = median(bestmodel.ef.data$visualAlpha_contra_nontarget),
  visualAlpha_contra_target = seq(min(bestmodel.ef.data$visualAlpha_contra_target),max(bestmodel.ef.data$visualAlpha_contra_target),1),
  SSVEP_contra_target = median(bestmodel.ef.data$SSVEP_contra_target),
  # subject = as.factor(unique(bestmodel.ef.data$subject))
  subject = as.factor(c(120)) # for a 'new' subject
)
...
theme_set(theme_bw())
g <- ggplot(bestmodel.ef.tidy_exppred2, aes(x = visualAlpha_contra_target, y = .epred)) +
  stat_lineribbon(.width = c(0.5, 0.8, 0.95, 1)) + 
  scale_fill_brewer(palette = "Reds") +
  facet_grid(cue_validity_label~.)+
  labs(x = "alpha amplitude contra target", y = "predicted RT",
...
# 
print(g)
# ggsave(filename = "figures/MarginEffects_PostCue_FullModelVisAlphaSSVEP_AlphaContraTarget_Slopes.eps",
#        width = 4, height = 4,
#        plot = print(g))
...
bestmodel.ef.newdata5 <- expand_grid(
  cue_validity_label = c('valid','neutral','invalid'),
  visualAlpha_contra_target = median(bestmodel.ef.data$visualAlpha_contra_target),
  visualAlpha_contra_nontarget = seq(min(bestmodel.ef.data$visualAlpha_contra_nontarget),max(bestmodel.ef.data$visualAlpha_contra_nontarget),1),
  SSVEP_contra_target = median(bestmodel.ef.data$SSVEP_contra_target),
  # subject = as.factor(unique(bestmodel.ef.data$subject))
  subject = as.factor(c(120)) # for a 'new' subject
)
...
theme_set(theme_bw())
g <- ggplot(bestmodel.ef.tidy_exppred3, aes(x = visualAlpha_contra_nontarget, y = .epred)) +
  stat_lineribbon(.width = c(0.5, 0.8, 0.95, 1)) + 
  scale_fill_brewer(palette = "Blues") +
  facet_grid(cue_validity_label~.)+
  labs(x = "alpha amplitude contra nontarget", y = "predicted RT",
...
print(g)
# ggsave(filename = "figures/MarginEffects_PostCue_FullModelVisAlphaSSVEP_AlphaContraNonTarget_Slopes.eps",
#        width = 4, height = 4,
#        plot = print(g))
...
bestmodel.ef.newdata5 <- expand_grid(
  ...
  visualAlpha_contra_target = median(bestmodel.ef.data$visualAlpha_contra_target),
  visualAlpha_contra_nontarget = median(bestmodel.ef.data$visualAlpha_contra_nontarget),
  SSVEP_contra_target = seq(min(bestmodel.ef.data$SSVEP_contra_target),max(bestmodel.ef.data$SSVEP_contra_target),1),
  # subject = as.factor(unique(bestmodel.ef.data$subject))
  subject = as.factor(c(120)) # for a 'new' subject
)
...
theme_set(theme_bw())
g <- ggplot(bestmodel.ef.tidy_exppred3, aes(x = SSVEP_contra_target, y = .epred)) +
  stat_lineribbon(.width = c(0.5, 0.8, 0.95, 1)) + 
  scale_fill_brewer(palette = "Oranges") +
  facet_grid(cue_validity_label~.)+
  labs(x = "SSVEP amplitude contra target", y = "predicted RT",
...
print(g)
# ggsave(filename = "figures/MarginEffects_PostCue_FullModelVisAlphaSSVEP_SSVEP_Slopes.eps",
#        width = 4, height = 4,
#        plot = print(g))
...
# now lets get the slop of the modulation by alpha
bestmodel.ef.slope_visualAlpha_contra_target <- bestmodel.ef.model %>% 
  emtrends(~ cue_validity_label, # 1 means, we want the average slop
           var = "visualAlpha_contra_target",
           epred = TRUE) %>% 
  gather_emmeans_draws()

bestmodel.ef.slope_visualAlpha_contra_nontarget <- bestmodel.ef.model %>% 
  emtrends(~ cue_validity_label, # 1 means, we want the average slop
           var = "visualAlpha_contra_nontarget",
           epred = TRUE) %>% 
  gather_emmeans_draws()

bestmodel.ef.slope_SSVEP_contra_target <- bestmodel.ef.model %>% 
  emtrends(~ cue_validity_label, # 1 means, we want the average slop
           var = "SSVEP_contra_target",
           epred = TRUE) %>% 
  gather_emmeans_draws()

bestmodel.ef.slopes_signals <- bind_rows(
  "visualAlpha_contra_target" = bestmodel.ef.slope_visualAlpha_contra_target,
  "visualAlpha_contra_nontarget" = bestmodel.ef.slope_visualAlpha_contra_nontarget,
  "SSVEP_contra_target" = bestmodel.ef.slope_SSVEP_contra_target,
  .id = "signal") %>% 
  mutate(signal = fct_inorder(signal))


theme_set(theme_bw())
g<-ggplot(bestmodel.ef.slopes_signals, aes( x = .value, y=cue_validity_label)) +
  stat_halfeye(
    aes(fill = signal, fill_ramp = after_stat(level)),
    .width = c(.95, 1),
  ) +
  # stat_dots(aes(fill = alpha_signal,color = alpha_signal, fill_ramp = after_stat(level), colour_ramp = after_stat(level)),
  #           .width = c(.95, 1))+
  geom_vline(xintercept = 0)+
  # facet_grid(cue_validity_label~.)+
  scale_fill_manual(values=c("red3","steelblue","gold")) +
  scale_color_manual(values=c("red3", "steelblue","gold"))+
  labs(
    title = "Average marginal effect: alpha slopes", 
    subtitle = "for all draws of the model",
    fill_ramp = "interval",
...
print(g)
ggsave(filename = "figures/MarginEffects_PostCue_FullModelVisAlpha_Alpha_SSVEP_SlopeDistr.eps",
       width = 4, height = 4,
       plot = print(g))


bestmodel.ef.slope_visualAlpha_contra_target %>% median_hdi() %>%
  kable(escape = F, digits = c(3,3,3,3,3,3,6,6,6), caption = c("alpha contratarget: mean and CI")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))

bestmodel.ef.slope_visualAlpha_contra_nontarget %>% median_hdi() %>%
  kable(escape = F, digits = c(3,3,3,3,3,3,6,6,6), caption = c("alpha contranontarget: mean and CI")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))

bestmodel.ef.slope_SSVEP_contra_target %>% median_hdi() %>%
  kable(escape = F, digits = c(3,3,3,3,3,3,6,6,6), caption = c("alpha contranontarget: mean and CI")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))


# contrast difference
loo_compare(fit_validity_x_alpha1_alpha2,  fit_validity_x_alpha1_alpha2_SSVEP,
            criterion = c("loo"))%>%
  kable(escape = F, digits = c(9,9,9,9,9,9,9,9,9), caption = c("alpha model vs alpha SSVEP model | loo comparison")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
