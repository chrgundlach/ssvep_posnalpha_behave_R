...
# only 24 Hz SSVEP on left side
modeldata4 = DATA.In_longer %>%
  filter(post_hit == "hit", signal != "motorAlpha", time == "[500 1500]ms", side == "leftStim")%>%
  dplyr::select(-Zamplitude)%>%
  pivot_wider(names_from = c(signal), values_from = amplitude)
...
fit_EEG_rightSSVEP_validity_x_alpha = brm(formula = SSVEP ~ attention * visualAlpha + (attention|subject),
          data = modeldata4, 
          family = shifted_lognormal(),
          file = 'fit_EEGv2_rightSSVEP_validity_x_alpha_nonoress_slog',
          save_pars = save_pars(all = TRUE),
          # file_refit = "on_change",
          # file_refit = "always",
          file_refit = "never",
...
# only 22 Hz SSVEP on right side
modeldata5 = DATA.In_longer %>%
  filter(post_hit == "hit", signal != "motorAlpha", time == "[500 1500]ms", side == "rightStim")%>%
  dplyr::select(-Zamplitude)%>%
  pivot_wider(names_from = c(signal), values_from = amplitude)
...
fit_EEG_leftSSVEP_validity_x_alpha = brm(formula = SSVEP ~ attention * visualAlpha + (attention|subject),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_EEGv2_leftSSVEP_validity_x_alpha_nonoress_slog',
          save_pars = save_pars(all = TRUE),
          file_refit = "on_change",
          # file_refit = "always",
          # file_refit = "never",
...
fit_EEG_rightSSVEP_validity_x_alpha <- add_criterion(fit_EEG_rightSSVEP_validity_x_alpha, c("loo", "waic", "bayes_R2"))

fit_EEG_leftSSVEP_validity_x_alpha <- add_criterion(fit_EEG_leftSSVEP_validity_x_alpha, c("loo", "waic", "bayes_R2"))
```
...
as.data.frame(rbind(
  ...
  bayes_R2(fit_EEG_rightSSVEP_validity_x_alpha),
  bayes_R2(fit_EEG_leftSSVEP_validity_x_alpha)),
  row.names = c( "fit_EEG_vis_alpha_validity", 
                "fit_EEG_mot_alpha_validity", 
                "fit_EEG_SSVEP_validity_x_alpha",
                "fit_EEG_rightSSVEP_validity_x_alpha",
                "fit_EEG_leftSSVEP_validity_x_alpha"))%>%
...
summary(fit_EEG_rightSSVEP_validity_x_alpha)
summary(fit_EEG_leftSSVEP_validity_x_alpha)
```
...
theme_set(theme_bw())
g <- ggplot(bestmodel.ef.tidy_exppred2, aes(x = visualAlpha, y = .epred)) +
...
bestmodel.ef.slope_contraTarget <- bestmodel.ef.model %>% 
  ...
  gather_emmeans_draws()
print(g)
# ggsave(filename = "MarginEffects_SSVEP_predictedby_ALPHA_RegressLine.eps", width = 5, height = 3.5,
#        plot = g)



theme_set(theme_bw())
g <- ggplot(bestmodel.ef.slope_contraTarget, aes( x = .value, y=attention)) +
  stat_halfeye(
    aes(fill = attention, fill_ramp = after_stat(level)),
    .width = c(.95, 1),
  ) +
...
print(g)
# ggsave(filename = "MarginEffects_SSVEP_predictedby_ALPHA_Slopes.eps", width = 5, height = 3.5,
#        plot = g)
...
##  Extract marginal effects from right SSVEP complex model alpha model


```{r rightSSVEP compl model marginal effects disp effects,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# based on https://www.andrewheiss.com/blog/2021/11/10/ame-bayes-re-guide/
# which model to use?
bestmodel.ef.model <- fit_EEG_rightSSVEP_validity_x_alpha
...
```{r rightSSVEP compl model marginal effects disp effects VALIDITY -data,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# use range of data from orignal study 
bestmodel.ef.newdata <- expand_grid(
  attention = as.factor(c('uncued','neutral','cued')),
  visualAlpha = median(bestmodel.ef.data$visualAlpha),
...
```{r rightSSVEP compl model marginal effects disp effects VALIDITY -marginal means,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
...
```{r rightSSVEP compl marginal effects disp effects visual alpha -marginal means,  fig.height=4, fig.width=4,  warning = FALSE, message=FALSE, eval = TRUE}
# now trying to get the regression line right for the alpha predictors
...
bestmodel.ef.slope_contraTarget <- bestmodel.ef.model %>% 
  ...
  gather_emmeans_draws()
print(g)
# ggsave(filename = "figures/MarginEffects_rightSSVEP_predictedby_ALPHA_RegressLine_rev02.eps", width = 4, height = 4,
#        plot = g)
...
g <- ggplot(bestmodel.ef.slope_contraTarget, aes( x = .value, y=attention)) +
  ...
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())
print(g)
ggsave(filename = "figures/MarginEffects_rightSSVEP_predictedby_ALPHA_Slopes_rev02.eps", width = 4, height = 4,
       plot = g)
...
##  Extract marginal effects from left SSVEP complex model alpha model


```{r leftSSVEP compl model marginal effects disp effects,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# based on https://www.andrewheiss.com/blog/2021/11/10/ame-bayes-re-guide/
# which model to use?
bestmodel.ef.model <- fit_EEG_leftSSVEP_validity_x_alpha
...
```{r leftSSVEP compl model marginal effects disp effects VALIDITY -data,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
# use range of data from orignal study 
bestmodel.ef.newdata <- expand_grid(
  attention = as.factor(c('uncued','neutral','cued')),
  visualAlpha = median(bestmodel.ef.data$visualAlpha),
...
```{r leftSSVEP compl model marginal effects disp effects VALIDITY -marginal means,  fig.height=3.5, fig.width=5,  warning = FALSE, message=FALSE, eval = TRUE}
...
```{r leftSSVEP compl marginal effects disp effects visual alpha -marginal means,  fig.height=4, fig.width=4,  warning = FALSE, message=FALSE, eval = TRUE}
# now trying to get the regression line right for the alpha predictors
