---
title: "BRMS analysis | Posnalpha Behave | cue window | revision: no RESS component excluded + zscored (only for subjects)"
...
modeldata4 = DATA.In_longer %>%
  ...
  group_by(subject)%>%
  mutate("contra_target"=scale(contra_target), "contra_nontarget"=scale(contra_nontarget)) %>%
  ungroup()
...
fit_validity_x_alpha1_alpha2 = brm(
  formula = post_RT ~ cue_validity_label * contra_target + cue_validity_label * contra_nontarget + (cue_validity_label|subject),
          data = modeldata4, 
          family = shifted_lognormal(),
          file = 'fit_validity_x_visalpha1_visalpha2_nonoress_zscore3_slog',
          save_pars = save_pars(all = TRUE),
          # file_refit = "on_change",
          # file_refit = "always",
          file_refit = "never",
...
fit_validity_x_SSVEP = brm(formula = post_RT ~ cue_validity_label * post_cue_SSVEP_contra_target + (cue_validity_label|subject),
          data = modeldata5, 
          family = shifted_lognormal(),
          file = 'fit_validity_x_ssvep_nonoress_zscore3_slog',
          save_pars = save_pars(all = TRUE),
          # file_refit = "on_change",
          # file_refit = "always",
          file_refit = "never",
...
fit_validity_x_motoralpha = brm(formula = post_RT ~ cue_validity_label * malpha_contra_target + (cue_validity_label|subject),
          data = modeldata7, 
          family = shifted_lognormal(),
          file = 'fit_validity_x_motoralpha_nonoress_zscore3_slog',
          save_pars = save_pars(all = TRUE),
          # file_refit = "on_change",
          # file_refit = "always",
          file_refit = "never",
...
bestmodel.ef.newdata2 <- expand_grid(
  ...
  contra_target = seq(-max(abs(bestmodel.ef.data$contra_target)),max(abs(bestmodel.ef.data$contra_target)),1),
  # contra_nontarget = seq(min(bestmodel.ef.data$contra_nontarget),max(bestmodel.ef.data$contra_nontarget),1),
  # subject = as.factor(unique(bestmodel.ef.data$subject))
  subject = as.factor(c(120)) # for a 'new' subject
)
...
# some add-on
bestmodel.ef.slopes_alphav2 <- bestmodel.ef.slopes_alpha %>%
  pivot_wider(names_from = alpha_signal, values_from = .value)%>%
  mutate(alpha_slope_diff = contra_target - contra_nontarget)

theme_set(theme_bw())
g<-ggplot(bestmodel.ef.slopes_alphav2, aes( x = alpha_slope_diff, y=cue_validity_label)) +
  stat_halfeye(
    aes(fill = cue_validity_label, fill_ramp = after_stat(level)),
    .width = c(.95, 1),
  ) +
  ...
  labs(
    title = "Avg marg eff: alpha slope diff", 
    subtitle = "contra_target - contra_nontarget",
    fill_ramp = "interval",
    colour_ramp = "interval",
    x = "slope"
  )
