...
datafile1 <- "C:/Users/psy05cvd/Dropbox/work/R-statistics/experiments/ssvep_posnalpha_behave/data_in/Behavior_FFT_singletrials_cue.txt"
...
eff_size(post_hocs.SSVEP$contrasts,
         sigma = sqrt(mean(sigma(anovamodel_SSVEP$lm)^2)), # RMS of sigma()
         edf = df.residual(anovamodel_SSVEP$lm), method = "identity")
...
summary(post_hocs.SSVEP$contrasts) %>%
  ...
  mutate(`CohensD` =
           summary(eff_size(post_hocs.SSVEP$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_SSVEP$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_SSVEP$lm), method = "identity"))$effect.size,
         `lowCI` =
           summary(eff_size(post_hocs.SSVEP$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_SSVEP$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_SSVEP$lm), method = "identity"))$lower.CL,
         `highCI` =
           summary(eff_size(post_hocs.SSVEP$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_SSVEP$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_SSVEP$lm), method = "identity"))$upper.CL
  ) %>%
...
summary(post_hocs.visalpha$contrasts) %>%
  ...
  # mutate(CohensD = `t.ratio`/sqrt(df+1))%>%
  mutate(`CohensD` =
           summary(eff_size(post_hocs.visalpha$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_visalpha$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_visalpha$lm), method = "identity"))$effect.size,
         `lowCI` =
           summary(eff_size(post_hocs.visalpha$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_visalpha$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_visalpha$lm), method = "identity"))$lower.CL,
         `highCI` =
           summary(eff_size(post_hocs.visalpha$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_visalpha$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_visalpha$lm), method = "identity"))$upper.CL
  ) %>%
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), 
...
summary(post_hocs.motalpha$contrasts) %>%
  ...
  # mutate(CohensD = `t.ratio`/sqrt(df+1))%>%
  mutate(`CohensD` =
           summary(eff_size(post_hocs.motalpha$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_motalpha$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_motalpha$lm), method = "identity"))$effect.size,
         `lowCI` =
           summary(eff_size(post_hocs.motalpha$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_motalpha$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_motalpha$lm), method = "identity"))$lower.CL,
         `highCI` =
           summary(eff_size(post_hocs.motalpha$contrasts,
                            sigma = sqrt(mean(sigma(anovamodel_motalpha$lm)^2)), # RMS of sigma()
                            edf = df.residual(anovamodel_motalpha$lm), method = "identity"))$upper.CL
  ) %>%
  kable(escape = F, digits = c(3,3,3,3,3,3,3,3,3), 
